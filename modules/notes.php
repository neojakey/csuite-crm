...
if ( $action === 'delete' && $id ) {
    ...
    <main class="flex-1 md:ml-56 p-6 pt-20 md:pt-6 max-w-md">
        ...
        <div class="bg-zinc-900 border border-zinc-800 rounded-lg p-6">
            <p class="text-zinc-300 mb-2"><?= __( 'notes.confirm_delete' ) ?></p>
            <p class="text-zinc-100 font-medium mb-6"><?= htmlspecialchars( $note['title'] ?: mb_substr( $note['body'], 0, 60 ), ENT_QUOTES, 'UTF-8' ) ?></p>
            <form method="post" action="<?= BASE_URL ?>index.php?page=notes&action=delete&id=<?= $id ?>">
                ...
                <div class="flex gap-3">
                    ...
                    <a href="<?= BASE_URL ?>index.php?page=notes" class="bg-zinc-800 hover:bg-zinc-700 text-zinc-100 px-4 py-2 rounded-md text-sm"><?= __( 'ui.cancel' ) ?></a>
                </div>
            </form>
        </div>
    </main>
...
if ( in_array( $action, [ 'add', 'edit' ], true ) ) {
    ...
    <main class="flex-1 md:ml-56 p-6 pt-20 md:pt-6 max-w-2xl">
        <div class="flex items-center gap-3 mb-6">
            <a href="<?= BASE_URL ?>index.php?page=notes" class="text-zinc-400 hover:text-zinc-100 text-sm">&larr; <?= __( 'ui.back' ) ?></a>
        ...
        <div class="bg-zinc-900 border border-zinc-800 rounded-lg p-6">
            <form method="post" action="<?= BASE_URL ?>index.php?page=notes&action=<?= $action ?><?= $id ? '&id=' . $id : '' ?>">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs text-zinc-400 mb-1"><?= __( 'notes.note_title' ) ?></label>
                    ...
                    <div>
                        <label class="block text-xs text-zinc-400 mb-1"><?= __( 'notes.body' ) ?> <span class="text-red-400">*</span></label>
                        <textarea name="body" rows="8" required
                                  class="bg-zinc-800 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-100 w-full focus:outline-none focus:border-orange-500 resize-y"><?= htmlspecialchars( $note['body'] ?? '', ENT_QUOTES, 'UTF-8' ) ?></textarea>
                    </div>
                    <div>
                        <label class="block text-xs text-zinc-400 mb-1"><?= __( 'notes.linked_contact' ) ?></label>
...
<main class="flex-1 md:ml-56 p-6 pt-20 md:pt-6">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="<?= BASE_URL ?>index.php?page=notes&action=add"
           class="bg-orange-500 hover:bg-orange-400 text-zinc-950 font-medium px-4 py-2 rounded-md text-sm"><?= __( 'notes.add' ) ?></a>
    </div>
    ...
    <?php if ( empty( $notes ) ) : ?>
    <div class="bg-zinc-900 border border-zinc-800 rounded-lg p-8 text-center text-zinc-500"><?= __( 'notes.none' ) ?></div>
    <?php else : ?>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <?php foreach ( $notes as $note ) : ?>
        <div class="bg-zinc-900 border border-zinc-800 rounded-lg p-4 flex flex-col">
            <div class="flex items-start justify-between mb-2">
                <h3 class="text-sm font-semibold text-zinc-200"><?= htmlspecialchars( $note['title'] ?: '—', ENT_QUOTES, 'UTF-8' ) ?></h3>
                <div class="flex gap-2 ml-2 shrink-0">
                    ...
                    <a href="<?= BASE_URL ?>index.php?page=notes&action=delete&id=<?= $note['id'] ?>" class="text-xs text-zinc-500 hover:text-red-400"><?= __( 'ui.delete' ) ?></a>
                </div>
            </div>
            <p class="text-sm text-zinc-400 flex-1 line-clamp-4"><?= nl2br( htmlspecialchars( $note['body'], ENT_QUOTES, 'UTF-8' ) ) ?></p>
            <?php if ( $note['company_name'] ) : ?>
            <p class="text-xs text-orange-500 mt-2"><?= htmlspecialchars( $note['company_name'], ENT_QUOTES, 'UTF-8' ) ?></p>
            <?php endif; ?>
            <p class="text-xs text-zinc-700 mt-1"><?= htmlspecialchars( $note['created_at'], ENT_QUOTES, 'UTF-8' ) ?></p>
        </div>
        <?php endforeach; ?>
    </div>
    <?php endif; ?>
