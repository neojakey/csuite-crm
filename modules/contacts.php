...
if ( $action === 'view' && $id ) {
    ...
    <main class="flex-1 md:ml-56 p-6 pt-20 md:pt-6">
        <div class="flex items-center gap-3 mb-6">
            <a href="<?= BASE_URL ?>index.php?page=contacts" class="text-slate-400 hover:text-slate-100 text-sm">&larr; <?= __( 'ui.back' ) ?></a>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
            <div class="lg:col-span-2 bg-slate-800 border border-slate-700 rounded-lg p-5">
                ...
                ?>
                <div class="flex py-2 border-b border-slate-700/50 last:border-0">
                    <span class="w-36 text-xs text-slate-500 uppercase tracking-wide pt-0.5 shrink-0"><?= $label ?></span>
                    <span class="text-sm text-slate-300"><?= htmlspecialchars( $contact[ $field ], ENT_QUOTES, 'UTF-8' ) ?></span>
                </div>
                <?php endforeach; ?>
            </div>
            <div class="bg-slate-800 border border-slate-700 rounded-lg p-5 space-y-3">
                ...
                $status_colours = [
                    ...
                    'dormant'  => 'bg-slate-600/30 text-slate-500',
                    'lost'     => 'bg-red-500/20 text-red-400',
                ];
                $sc = $status_colours[ $contact['status'] ] ?? 'bg-slate-500/20 text-slate-300';
                ?>
                ...
                <div class="flex flex-col gap-2 pt-2">
                    <a href="<?= BASE_URL ?>index.php?page=contacts&action=edit&id=<?= $contact['id'] ?>"
                       class="bg-slate-700 hover:bg-slate-600 text-slate-100 px-4 py-2 rounded-md text-sm text-center transition-colors"><?= __( 'ui.edit' ) ?></a>
        ...
        <!-- Linked notes -->
        <div class="bg-slate-800 border border-slate-700 rounded-lg p-5 mb-4">
            <div class="flex items-center justify-between mb-3">
                <h2 class="text-sm font-semibold text-slate-300"><?= __( 'notes.title' ) ?></h2>
                <a href="<?= BASE_URL ?>index.php?page=notes&action=add&contact_id=<?= $contact['id'] ?>" class="text-xs text-cyan-500 hover:text-cyan-400"><?= __( 'notes.add' ) ?></a>
            </div>
            <?php if ( empty( $linked_notes ) ) : ?>
            <p class="text-sm text-slate-500"><?= __( 'notes.none' ) ?></p>
            <?php else : ?>
            <div class="space-y-2">
                <?php foreach ( $linked_notes as $note ) : ?>
                <div class="py-2 border-b border-slate-700/50 last:border-0">
                    <div class="flex justify-between">
                        <p class="text-sm font-medium text-slate-300"><?= htmlspecialchars( $note['title'] ?: '—', ENT_QUOTES, 'UTF-8' ) ?></p>
                    ...
                    <p class="text-sm text-slate-400 mt-1"><?= nl2br( htmlspecialchars( $note['body'], ENT_QUOTES, 'UTF-8' ) ) ?></p>
                </div>
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
        ...
        <div class="bg-slate-800 border border-slate-700 rounded-lg p-5 mb-4">
            <div class="flex items-center justify-between mb-3">
                <h2 class="text-sm font-semibold text-slate-300"><?= __( 'tasks.title' ) ?></h2>
                <a href="<?= BASE_URL ?>index.php?page=tasks&action=add&contact_id=<?= $contact['id'] ?>" class="text-xs text-cyan-500 hover:text-cyan-400"><?= __( 'tasks.add' ) ?></a>
            </div>
            <?php if ( empty( $linked_tasks ) ) : ?>
            <p class="text-sm text-slate-500"><?= __( 'tasks.none' ) ?></p>
            <?php else : ?>
            <div class="space-y-2">
                <?php foreach ( $linked_tasks as $task ) : ?>
                <div class="flex items-center gap-3 py-2 border-b border-slate-700/50 last:border-0">
                    <span class="text-xs px-2 py-0.5 rounded bg-slate-600/50 text-slate-400"><?= __( 'tasks.priority.' . $task['priority'] ) ?></span>
                    <span class="flex-1 text-sm text-slate-300"><?= htmlspecialchars( $task['title'], ENT_QUOTES, 'UTF-8' ) ?></span>
                    <?php if ( $task['due_date'] ) : ?><span class="text-xs text-slate-500"><?= htmlspecialchars( $task['due_date'], ENT_QUOTES, 'UTF-8' ) ?></span><?php endif; ?>
                </div>
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
        ...
        <div class="bg-slate-800 border border-slate-700 rounded-lg p-5">
            <h2 class="text-sm font-semibold text-slate-300 mb-3"><?= __( 'agents.history' ) ?></h2>
            <div class="space-y-2">
                <?php foreach ( $linked_sessions as $s ) : ?>
                <div class="py-2 border-b border-slate-700/50 last:border-0">
                    <div class="flex items-center gap-2 mb-1">
                        <span class="text-xs px-2 py-0.5 bg-cyan-500/20 text-cyan-400 rounded"><?= htmlspecialchars( $s['agent_role'], ENT_QUOTES, 'UTF-8' ) ?></span>
                        <span class="text-xs text-slate-500"><?= htmlspecialchars( $s['created_at'], ENT_QUOTES, 'UTF-8' ) ?></span>
                    </div>
                    <p class="text-sm text-slate-400 truncate"><?= htmlspecialchars( $s['user_prompt'], ENT_QUOTES, 'UTF-8' ) ?></p>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
...
if ( $action === 'delete' && $id ) {
    ...
    <main class="flex-1 md:ml-56 p-6 pt-20 md:pt-6 max-w-lg">
        ...
        <div class="bg-slate-800 border border-slate-700 rounded-lg p-6">
            <p class="text-slate-300 mb-2"><?= __( 'contacts.confirm_delete' ) ?></p>
            <p class="text-sm text-amber-400 mb-6"><?= __( 'contacts.gdpr_notice' ) ?></p>
            <p class="text-slate-100 font-medium mb-6"><?= htmlspecialchars( $contact['company_name'], ENT_QUOTES, 'UTF-8' ) ?></p>
            <form method="post" action="<?= BASE_URL ?>index.php?page=contacts&action=delete&id=<?= $id ?>">
                ...
                <div class="flex gap-3">
                    ...
                    <a href="<?= BASE_URL ?>index.php?page=contacts" class="bg-slate-700 hover:bg-slate-600 text-slate-100 px-4 py-2 rounded-md text-sm"><?= __( 'ui.cancel' ) ?></a>
                </div>
            </form>
        </div>
    </main>
            ...
            <form method="post" action="<?= BASE_URL ?>index.php?page=contacts&action=<?= $action ?><?= $id ? '&id=' . $id : '' ?>">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2">
                        <label class="block text-xs text-slate-400 mb-1"><?= __( 'contacts.company_name' ) ?> <span class="text-red-400">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs text-slate-400 mb-1"><?= __( 'contacts.contact_name' ) ?></label>
                        <input type="text" name="contact_name" value="<?= htmlspecialchars( $contact['contact_name'] ?? '', ENT_QUOTES, 'UTF-8' ) ?>"
                               class="bg-slate-700 border border-slate-600 rounded-md px-3 py-2 text-sm text-slate-100 w-full focus:outline-none focus:border-cyan-500">
                    </div>
                    <div>
                        <label class="block text-xs text-slate-400 mb-1"><?= __( 'contacts.email' ) ?></label>
                        <input type="email" name="email" value="<?= htmlspecialchars( $contact['email'] ?? '', ENT_QUOTES, 'UTF-8' ) ?>"
                               class="bg-slate-700 border border-slate-600 rounded-md px-3 py-2 text-sm text-slate-100 w-full focus:outline-none focus:border-cyan-500">
                    </div>
                    <div>
                        <label class="block text-xs text-slate-400 mb-1"><?= __( 'contacts.phone' ) ?></label>
                        <input type="tel" name="phone" value="<?= htmlspecialchars( $contact['phone'] ?? '', ENT_QUOTES, 'UTF-8' ) ?>"
                               class="bg-slate-700 border border-slate-600 rounded-md px-3 py-2 text-sm text-slate-100 w-full focus:outline-none focus:border-cyan-500">
                    </div>
                    <div>
                        <label class="block text-xs text-slate-400 mb-1"><?= __( 'contacts.website' ) ?></label>
                        <input type="url" name="website" value="<?= htmlspecialchars( $contact['website'] ?? '', ENT_QUOTES, 'UTF-8' ) ?>"
                               class="bg-slate-700 border border-slate-600 rounded-md px-3 py-2 text-sm text-slate-100 w-full focus:outline-none focus:border-cyan-500">
                    </div>
                    <div>
                        <label class="block text-xs text-slate-400 mb-1"><?= __( 'contacts.source' ) ?></label>
                        <select name="source" class="bg-slate-700 border border-slate-600 rounded-md px-3 py-2 text-sm text-slate-100 w-full focus:outline-none focus:border-cyan-500">
                    ...
                    <div>
                        <label class="block text-xs text-slate-400 mb-1"><?= __( 'contacts.status' ) ?></label>
                    ...
                    <div>
                        <label class="block text-xs text-slate-400 mb-1"><?= __( 'contacts.pipeline_stage' ) ?></label>
                        <input type="text" name="pipeline_stage" value="<?= htmlspecialchars( $contact['pipeline_stage'] ?? '', ENT_QUOTES, 'UTF-8' ) ?>"
                               class="bg-slate-700 border border-slate-600 rounded-md px-3 py-2 text-sm text-slate-100 w-full focus:outline-none focus:border-cyan-500">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-xs text-slate-400 mb-1"><?= __( 'contacts.notes' ) ?></label>
                        <textarea name="notes" rows="4"
                                  class="bg-slate-700 border border-slate-600 rounded-md px-3 py-2 text-sm text-slate-100 w-full focus:outline-none focus:border-cyan-500 resize-y"><?= htmlspecialchars( $contact['notes'] ?? '', ENT_QUOTES, 'UTF-8' ) ?></textarea>
                    </div>
                </div>
...
<main class="flex-1 md:ml-56 p-6 pt-20 md:pt-6">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="<?= BASE_URL ?>index.php?page=contacts&action=add"
           class="bg-cyan-500 hover:bg-cyan-400 text-slate-900 font-medium px-4 py-2 rounded-md text-sm transition-colors"><?= __( 'contacts.add' ) ?></a>
    </div>
    <?php if ( $flash = get_flash( 'success' ) ) : ?><div class="mb-4 px-4 py-3 bg-green-500/10 border border-green-500/30 rounded-md text-green-400 text-sm flash-message"><?= $flash ?></div><?php endif; ?>
    <?php if ( $flash = get_flash( 'error' ) ) : ?><div class="mb-4 px-4 py-3 bg-red-500/10 border border-red-500/30 rounded-md text-red-400 text-sm flash-message"><?= $flash ?></div><?php endif; ?>
    ...
    <form method="get" action="<?= BASE_URL ?>index.php" class="flex flex-wrap gap-3 mb-4">
        ...
        <input type="text" name="search" placeholder="<?= __( 'ui.search' ) ?>..." value="<?= htmlspecialchars( $filters['search'], ENT_QUOTES, 'UTF-8' ) ?>"
               class="bg-slate-800 border border-slate-600 rounded-md px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-cyan-500 w-48">
        <select name="status" class="bg-slate-800 border border-slate-600 rounded-md px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-cyan-500">
            <option value=""><?= __( 'ui.all' ) ?></option>
            <?php foreach ( [ 'prospect', 'warm', 'active', 'customer', 'dormant', 'lost' ] as $st ) : ?>
            <option value="<?= $st ?>" <?= $filters['status'] === $st ? 'selected' : '' ?>><?= __( 'contacts.status.' . $st ) ?></option>
            <?php endforeach; ?>
        </select>
        <button type="submit" class="bg-slate-700 hover:bg-slate-600 text-slate-100 px-4 py-2 rounded-md text-sm"><?= __( 'ui.filter' ) ?></button>
        <?php if ( $filters['search'] || $filters['status'] ) : ?>
        <a href="<?= BASE_URL ?>index.php?page=contacts" class="text-slate-400 hover:text-slate-100 px-3 py-2 text-sm">&times; <?= __( 'ui.all' ) ?></a>
        <?php endif; ?>
    </form>
    ...
    <div class="bg-slate-800 border border-slate-700 rounded-lg overflow-hidden">
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr><td colspan="5" class="px-4 py-8 text-center text-slate-500"><?= __( 'contacts.none' ) ?></td></tr>
                <?php else :
                $status_colours = [
                    'prospect' => 'bg-slate-500/20 text-slate-300',
                    'warm'     => 'bg-amber-500/20 text-amber-400',
                ...
                foreach ( $contacts as $c ) :
                    $sc = $status_colours[ $c['status'] ] ?? 'bg-slate-500/20 text-slate-300';
                ?>
                <tr class="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                    <td class="px-4 py-3">
                        <a href="<?= BASE_URL ?>index.php?page=contacts&action=view&id=<?= $c['id'] ?>" class="text-cyan-400 hover:text-cyan-300 font-medium">
                            <?= htmlspecialchars( $c['company_name'], ENT_QUOTES, 'UTF-8' ) ?>
                        </a>
                    </td>
                    <td class="px-4 py-3 text-slate-300 hidden md:table-cell"><?= htmlspecialchars( $c['contact_name'] ?? '', ENT_QUOTES, 'UTF-8' ) ?></td>
                    <td class="px-4 py-3 text-slate-400 hidden lg:table-cell"><?= htmlspecialchars( $c['email'] ?? '', ENT_QUOTES, 'UTF-8' ) ?></td>
                    <td class="px-4 py-3"><span class="px-2 py-0.5 rounded text-xs <?= $sc ?>"><?= __( 'contacts.status.' . $c['status'] ) ?></span></td>
                    <td class="px-4 py-3 text-right">
                        <a href="<?= BASE_URL ?>index.php?page=contacts&action=edit&id=<?= $c['id'] ?>" class="text-slate-400 hover:text-cyan-400 text-xs mr-3"><?= __( 'ui.edit' ) ?></a>
                        <a href="<?= BASE_URL ?>index.php?page=contacts&action=delete&id=<?= $c['id'] ?>" class="text-slate-400 hover:text-red-400 text-xs"><?= __( 'ui.delete' ) ?></a>
                    </td>
                </tr>
    ...
    <div class="flex items-center justify-between mt-4 text-sm text-slate-400">
        ...
        <div class="flex gap-2">
            ...
            <a href="?page=contacts&p=<?= $page_num - 1 ?>&status=<?= urlencode( $filters['status'] ) ?>&search=<?= urlencode( $filters['search'] ) ?>"
               class="bg-slate-700 hover:bg-slate-600 px-3 py-1 rounded-md"><?= __( 'ui.previous' ) ?></a>
            <?php endif; ?>
            <?php if ( $page_num < $total_pages ) : ?>
            <a href="?page=contacts&p=<?= $page_num + 1 ?>&status=<?= urlencode( $filters['status'] ) ?>&search=<?= urlencode( $filters['search'] ) ?>"
               class="bg-slate-700 hover:bg-slate-600 px-3 py-1 rounded-md"><?= __( 'ui.next' ) ?></a>
            <?php endif; ?>
        </div>
    </div>
    <?php endif; ?>
